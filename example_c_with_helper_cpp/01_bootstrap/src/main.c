...
struct VulkanContext {
    ...
    RenderObject cube;
    uint32_t graphicsQueueFamilyIndex;  // Added to store the queue family index
} vkCtx = {0};
...
/* Declaration of the Vulkan initialization function provided by vulkan_init.cpp */
...
void create_triangle() {
    ...
    if (vkAllocateMemory(vkCtx.device, &allocInfo, NULL, &vkCtx.triangle.memory) != VK_SUCCESS) {
        fprintf(stderr, "Failed to allocate triangle vertex buffer memory\n");
        exit(1);
    }
...
void create_cube() {
    ...
    if (vkCreateBuffer(vkCtx.device, &bufferInfo, NULL, &vkCtx.cube.buffer) != VK_SUCCESS) {
        fprintf(stderr, "Failed to create cube vertex buffer\n");
        exit(1);
    }
    ...
    if (vkAllocateMemory(vkCtx.device, &allocInfo, NULL, &vkCtx.cube.memory) != VK_SUCCESS) {
        fprintf(stderr, "Failed to allocate cube vertex buffer memory\n");
        exit(1);
    }
...
int main(int argc, char* argv[]) {
    ...
    commandPoolInfo.queueFamilyIndex = vkCtx.graphicsQueueFamilyIndex;  // Use the index from vk-bootstrap
