...
#include <cglm/cglm.h>
#include <vk_mem_alloc.h>  // Include VMA header for type definitions
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
...
typedef struct {
    VkBuffer buffer;
    VmaAllocation allocation;  // VMA allocation type
...
struct VulkanContext {
    ...
    VmaAllocation uniformAllocation;  // VMA allocation for uniform buffer
    ...
    VkImage depthImage;
    VmaAllocation depthAllocation;  // VMA allocation for depth image
...
extern VmaAllocator allocator;  // Declared in vulkan_init.cpp

uint32_t find_memory_type(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
    // Note: With VMA, this function is no longer needed as VMA handles memory types internally.
    // Keeping it for reference or future manual allocations if needed.
...
/* Declaration of the Vulkan initialization function */
...
void create_triangle() {
    ...
    allocInfo.usage = VMA_MEMORY_USAGE_CPU_TO_GPU;  // Suitable for staging data from CPU to GPU
...
int main(int argc, char* argv[]) {
    ...
    depthAllocInfo.usage = VMA_MEMORY_USAGE_GPU_ONLY;  // GPU-only for depth image
