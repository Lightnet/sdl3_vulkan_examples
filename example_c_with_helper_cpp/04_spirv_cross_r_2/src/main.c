// main.c: Main entry point for Vulkan SDL3 application
// Purpose: Initializes SDL and Vulkan, manages rendering loop, and handles input and cleanup
...
#include "vulkan_init.h" // Vulkan initialization functions and globals
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>

// Window dimensions (hardcoded for simplicity)
#define WIDTH 800
#define HEIGHT 600

// Camera structure for position and orientation control
typedef struct {
...
// Render object for vertex buffers (triangle or cube)
typedef struct {
    VkBuffer buffer;          // Vulkan buffer handle
    VmaAllocation allocation; // VMA memory allocation handle
    uint32_t vertexCount;     // Number of vertices in the buffer
    bool exists;              // Flag to track creation state
} RenderObject;

// Global Vulkan context to hold all Vulkan state
struct VulkanContext {
    ...
    RenderObject triangle; // Triangle vertex data
    RenderObject cube;     // Cube vertex data
    uint32_t graphicsQueueFamilyIndex;
} vkCtx = {0}; // Zero-initialize to avoid garbage values

// Uniform buffer object for shader matrices
typedef struct {
    mat4 model; // Model transformation (e.g., rotation)
    mat4 view;  // View transformation from camera
    mat4 proj;  // Perspective projection
} UBO;

// Utility function to find a suitable memory type for Vulkan allocations
// Parameters:
// - typeFilter: Bitmask of allowed memory types
// - properties: Required memory properties (e.g., host-visible)
...
// Parameters:
// - cam: Camera struct for view matrix
// - rotationAngle: Angle (degrees) for model rotation
void update_uniform_buffer(Camera* cam, float rotationAngle) {
    UBO ubo;
    glm_mat4_identity(ubo.model); // Reset model matrix to identity
    glm_rotate_y(ubo.model, glm_rad(rotationAngle), ubo.model); // Rotate around Y-axis
    glm_lookat(cam->pos, (vec3){cam->pos[0] + cam->front[0], cam->pos[1] + cam->front[1], cam->pos[2] + cam->front[2]}, cam->up, ubo.view); // Compute view matrix
    glm_perspective(glm_rad(45.0f), (float)WIDTH / HEIGHT, 0.1f, 100.0f, ubo.proj); // 45° FOV, near 0.1, far 100
    ubo.proj[1][1] *= -1; // Flip Y-axis for Vulkan’s top-left origin

    void* data;
    vmaMapMemory(allocator, vkCtx.uniformAllocation, &data); // Map buffer memory
    memcpy(data, &ubo, sizeof(UBO)); // Copy UBO data
    vmaUnmapMemory(allocator, vkCtx.uniformAllocation); // Unmap memory
}

// Creates a triangle vertex buffer
void create_triangle() {
    ...
    allocInfo.usage = VMA_MEMORY_USAGE_CPU_TO_GPU; // Host-accessible, GPU-readable
...
// Destroys the triangle vertex buffer
void destroy_triangle() {
    ...
    vkDeviceWaitIdle(vkCtx.device); // Wait for GPU to finish using the buffer
...
// Creates a cube vertex buffer (36 vertices for 12 triangles)
void create_cube() {
    ...
    // Vertex data: position (x, y, z), color (r, g, b) for 6 faces
    float vertices[] = {
        // Front face
        -0.5f, -0.5f,  0.5f,  1.0f, 0.0f, 0.0f,  0.5f,  0.5f,  0.5f,  0.0f, 0.0f, 1.0f,  0.5f, -0.5f,  0.5f,  0.0f, 1.0f, 0.0f,
        -0.5f, -0.5f,  0.5f,  1.0f, 0.0f, 0.0f, -0.5f,  0.5f,  0.5f,  1.0f, 1.0f, 0.0f,  0.5f,  0.5f,  0.5f,  0.0f, 0.0f, 1.0f,
        // Back face
        -0.5f, -0.5f, -0.5f,  1.0f, 0.0f, 1.0f,  0.5f, -0.5f, -0.5f,  0.0f, 1.0f, 1.0f,  0.5f,  0.5f, -0.5f,  1.0f, 1.0f, 1.0f,
        -0.5f, -0.5f, -0.5f,  1.0f, 0.0f, 1.0f,  0.5f,  0.5f, -0.5f,  1.0f, 1.0f, 1.0f, -0.5f,  0.5f, -0.5f,  0.5f, 0.5f, 0.5f,
        // Left face
        -0.5f, -0.5f,  0.5f,  1.0f, 0.0f, 0.0f, -0.5f, -0.5f, -0.5f,  1.0f, 0.0f, 1.0f, -0.5f,  0.5f, -0.5f,  0.5f, 0.5f, 0.5f,
        -0.5f, -0.5f,  0.5f,  1.0f, 0.0f, 0.0f, -0.5f,  0.5f, -0.5f,  0.5f, 0.5f, 0.5f, -0.5f,  0.5f,  0.5f,  1.0f, 1.0f, 0.0f,
        // Right face
         0.5f, -0.5f,  0.5f,  0.0f, 1.0f, 0.0f,  0.5f,  0.5f,  0.5f,  0.0f, 0.0f, 1.0f,  0.5f,  0.5f, -0.5f,  1.0f, 1.0f, 1.0f,
         0.5f, -0.5f,  0.5f,  0.0f, 1.0f, 0.0f,  0.5f,  0.5f, -0.5f,  1.0f, 1.0f, 1.0f,  0.5f, -0.5f, -0.5f,  0.0f, 1.0f, 1.0f,
        // Top face
        -0.5f,  0.5f,  0.5f,  1.0f, 1.0f, 0.0f, -0.5f,  0.5f, -0.5f,  0.5f, 0.5f, 0.5f,  0.5f,  0.5f, -0.5f,  1.0f, 1.0f, 1.0f,
        -0.5f,  0.5f,  0.5f,  1.0f, 1.0f, 0.0f,  0.5f,  0.5f, -0.5f,  1.0f, 1.0f, 1.0f,  0.5f,  0.5f,  0.5f,  0.0f, 0.0f, 1.0f,
        // Bottom face
        -0.5f, -0.5f, -0.5f,  1.0f, 0.0f, 1.0f, -0.5f, -0.5f,  0.5f,  1.0f, 0.0f, 0.0f,  0.5f, -0.5f,  0.5f,  0.0f, 1.0f, 0.0f,
        -0.5f, -0.5f, -0.5f,  1.0f, 0.0f, 1.0f,  0.5f, -0.5f,  0.5f,  0.0f, 1.0f, 0.0f,  0.5f, -0.5f, -0.5f,  0.0f, 1.0f, 1.0f
    };
...
// Destroys the cube vertex buffer
void destroy_cube() {
    if (!vkCtx.cube.exists) {
...
// Reflects vertex inputs from SPIR-V shader code using SPIRV-Cross
// Parameters:
// - shaderCode: SPIR-V bytecode buffer
// - codeSize: Size of the bytecode in bytes
// - attrDesc: Output array of vertex attribute descriptions
// - attrCount: Output number of attributes
// - stride: Output total stride (size per vertex)
...
#pragma warning(push)
#pragma warning(disable: 4047) // Suppress MSVC pointer indirection warning
...
// Creates the graphics pipeline with shaders and vertex input state
...
// Records commands to draw the triangle and cube
// Parameter:
// - imageIndex: Index of the swapchain image to render to
void record_command_buffer(uint32_t imageIndex) {
    ...
    VkClearValue clearValues[2] = {{{{0.5f, 0.5f, 0.5f, 1.0f}}}, {{{1.0f, 0}}}}; // Gray background, clear depth
...
void reset_camera(Camera* cam) {
    cam->pos[0] = 0.0f; cam->pos[1] = 0.0f; cam->pos[2] = 3.0f; // Position 3 units back
    cam->front[0] = 0.0f; cam->front[1] = 0.0f; cam->front[2] = -1.0f; // Facing -Z
    cam->up[0] = 0.0f; cam->up[1] = 1.0f; cam->up[2] = 0.0f; // Up is +Y
    cam->yaw = -90.0f; // Facing along -Z (left is +X)
    cam->pitch = 0.0f; // No vertical tilt
    printf("Camera reset: Pos [0, 0, 3], Yaw -90, Pitch 0\n");
}

// Handles camera movement and rotation based on input events
// Parameters:
// - cam: Camera struct with position, orientation, and angles
// - event: SDL event to process (key presses or mouse motion)
// - mouseCaptured: Flag to enable/disable mouse look
// - window: SDL window for setting relative mouse mode
// - deltaTime: Time since last frame (in milliseconds) for smooth movement
void update_camera(Camera* cam, SDL_Event* event, bool* mouseCaptured, SDL_Window* window, Uint32 deltaTime) {
    const float speed = 2.5f; // Base movement speed (units per second)
    float moveSpeed = speed * (deltaTime / 1000.0f); // Adjust speed by frame time for consistency

    // Handle keyboard input for movement
    if (event->type == SDL_EVENT_KEY_DOWN) {
        switch (event->key.key) {
            case SDLK_W: { // Move forward along camera's front direction
            ...
            case SDLK_A: { // Strafe left (perpendicular to front and up)
            ...
            case SDLK_ESCAPE: { // Release mouse capture
                *mouseCaptured = false;
    ...
    // Capture mouse with left click to enable look-around
    ...
    // Mouse look: Updates yaw (horizontal) and pitch (vertical) when captured
    if (*mouseCaptured && event->type == SDL_EVENT_MOUSE_MOTION) {
        float sensitivity = 0.1f; // Mouse sensitivity (degrees per pixel)

        // Update yaw (left/right rotation) based on horizontal mouse movement
        // Positive xrel (right) increases yaw, negative (left) decreases it
        cam->yaw += event->motion.xrel * sensitivity;

        // Update pitch (up/down rotation) with inverted vertical movement
        // Negative yrel (down) increases pitch (look down), positive (up) decreases it (look up)
        cam->pitch -= event->motion.yrel * sensitivity; // Inverted for natural FPS feel

        // Clamp pitch to prevent flipping (between -89° and 89°)
        if (cam->pitch > 89.0f) cam->pitch = 89.0f;
        if (cam->pitch < -89.0f) cam->pitch = -89.0f;

        // Recalculate front vector from yaw and pitch (spherical to Cartesian)
        vec3 front;
        front[0] = cosf(glm_rad(cam->yaw)) * cosf(glm_rad(cam->pitch)); // X: left/right and up/down
        front[1] = sinf(glm_rad(cam->pitch));                           // Y: up/down only
        front[2] = sinf(glm_rad(cam->yaw)) * cosf(glm_rad(cam->pitch)); // Z: left/right and up/down
        glm_vec3_normalize_to(front, cam->front); // Normalize to unit vector
...
int main(int argc, char* argv[]) {
    ...
    while (running) {
        ...
        if (rotateObjects) {
            rotationAngle += 90.0f * (deltaTime / 1000.0f); // Rotate at 90°/s
...
// Notes for Users:
// - WIDTH and HEIGHT are hardcoded; consider making them configurable.
// - vkCtx is global for simplicity; in larger apps, pass it as a parameter.
// - Cleanup order is critical: free VMA resources before allocator, allocator before device.
// - Shaders (vert.spv, frag.spv) must be in Debug/ directory at runtime.
// - Mouse look is inverted (up = look up); adjust pitch in update_camera() to revert.