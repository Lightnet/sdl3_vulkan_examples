...
// Load shader code from file (for simplicity, assuming SPIR-V binary is provided)
static VkShaderModule createShaderModule(VkDevice device, const char* filename) {
    ...
    checkVkResult(vkCreateShaderModule(device, &createInfo, NULL, &module), "vkCreateShaderModule");
    free(code);
    return module;
}
...
void createSwapchainAndPipeline(VulkanContext* ctx, SDL_Window* window) {
    ...
    // Create pipeline
    ctx->vertShaderModule = createShaderModule(ctx->device, "shaders/vert.spv");
    ctx->fragShaderModule = createShaderModule(ctx->device, "shaders/frag.spv");
