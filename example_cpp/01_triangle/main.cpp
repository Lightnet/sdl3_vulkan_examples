...
int main(int argc, char* argv[]) {
    ...
    if (memoryTypeIndex == UINT32_MAX) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to find suitable memory type for vertex buffer");
    ...
    if (vkAllocateMemory(device, &allocInfo, nullptr, &vertexBufferMemory) != VK_SUCCESS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to allocate vertex buffer memory");
        vkDestroyBuffer(device, vertexBuffer, nullptr);
        vkDestroyCommandPool(device, commandPool, nullptr);
        for (auto& fb : swapchainFramebuffers) vkDestroyFramebuffer(device, fb, nullptr);
        vkDestroyPipeline(device, graphicsPipeline, nullptr);
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Successfully created vertex buffer");
