...
static void updateUniformBuffer(VSDL_Context& ctx, float posX, float posY, float rotX, float rotY, float rotZ) {
    UniformBufferObject ubo = {};
    // Simple 2D transformation matrix (translation + rotation)
    float cosX = cosf(rotX), sinX = sinf(rotX);
    float cosY = cosf(rotY), sinY = sinf(rotY);
    float cosZ = cosf(rotZ), sinZ = sinf(rotZ);

    // 4x4 matrix: [R][T]
    ubo.transform[0] = cosZ * cosY;  // m00
    ubo.transform[1] = sinZ * cosY;  // m01
    ubo.transform[2] = -sinY;        // m02
    ubo.transform[3] = 0.0f;         // m03
    ubo.transform[4] = -sinZ * cosX + cosZ * sinY * sinX; // m10
    ubo.transform[5] = cosZ * cosX + sinZ * sinY * sinX;  // m11
    ubo.transform[6] = cosY * sinX;  // m12
    ubo.transform[7] = 0.0f;         // m13
    ubo.transform[8] = sinZ * sinX + cosZ * sinY * cosX;  // m20
    ubo.transform[9] = -cosZ * sinX + sinZ * sinY * cosX; // m21
    ubo.transform[10] = cosY * cosX; // m22
...
bool vsdl_render_loop(VSDL_Context& ctx) {
    ...
    while (running) {
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_EVENT_KEY_DOWN) {
                switch (event.key.key) {
                    ...
                    case SDLK_1: rotX += rotSpeed; break;
                    case SDLK_2: rotX -= rotSpeed; break;
                    case SDLK_3: rotY += rotSpeed; break;
                    case SDLK_4: rotY -= rotSpeed; break;
        ...
        updateUniformBuffer(ctx, posX, posY, rotX, rotY, rotZ);
