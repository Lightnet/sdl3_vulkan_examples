...
bool vsdl_render_loop(VSDL_Context& ctx) {
  ...
  SDL_ShowWindow(ctx.window);
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_EVENT_KEY_DOWN) {
                switch (event.key.key) { // Changed from event.key.keysym.sym
                    ...
                    case SDLK_1: create_vertex_buffer(ctx); break;
                    case SDLK_2: destroy_vertex_buffer(ctx); break;
                }
            }
        }
        ...
        for (const auto& bufferPair : ctx.vertexBuffers) { // Avoid structured binding
            VkBuffer buffer = bufferPair.first;
    ...
    for (auto& bufferPair : ctx.vertexBuffers) { // Avoid structured binding
        vmaDestroyBuffer(ctx.allocator, bufferPair.first, bufferPair.second);
    }
