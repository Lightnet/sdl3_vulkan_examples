#define SDL_MAIN_USE_CALLBACKS  // Define before any SDL headers
#include <SDL3/SDL.h>           // Core SDL functionality
#include <SDL3/SDL_main.h>      // Required for callback system
#include <ft2build.h>
...
SDL_Texture* create_text_texture(SDL_Renderer *renderer, FT_Library ft, const char *text, const char *font_path, int font_size) {
    ...
    const char *str = text;
    int width = 0, height = font_size;
    for (const char *c = str; *c; c++) {
        if (FT_Load_Char(face, *c, FT_LOAD_RENDER)) continue;
        width += face->glyph->bitmap.width + 2; // Add spacing
    }
    ...
    for (const char *c = str; *c; c++) {
        ...
        for (int y = 0; y < bitmap->rows; y++) {
            for (int x = 0; x < bitmap->width; x++) {
                ...
                if (alpha) {
                    SDL_Rect pixel = {x_offset + x, y, 1, 1};
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
    ...
    const char *font_path = "C:/Windows/Fonts/arial.ttf"; // Example for Windows
...
SDL_AppResult SDL_AppIterate(void *appstate) {
    ...
    SDL_FRect dst = {(640 - tex_w) / 2.0f, (480 - tex_h) / 2.0f, tex_w, tex_h};
